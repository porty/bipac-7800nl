 ...
 *
 * Note that the obj may contain fields that are marked
 * as read-only in the data model.  Even though the caller
 * may change those fields in his local copy of the object,
 * the changes to the read-only fields will be silently ignored.
...
/** Special interface for tr69c to set some of the management
 *  server object parameters.  (Specifically the read-only ParmeterKey).
 *
 * This function should only be used by tr69c.  This function will
 * allow all read-only parameters in this object to be written to.
 * This function does not trigger the RCL handler function.  The new
 * object will have its parameters checked against the restrictions in
 * the data model, and then be placed directly into the MDM.
 *
 * @param mdmObj   (IN) Pointer to the modified ManagementServerObject.
 *
 * @return CmsRet enum.
 */
CmsRet cmsObj_setMgmtServer(const ManagementServerObject *mdmObj);


/** Special interface for omcid to set the read-only ManagedEntityId
 *  parameters in the GPON objects.
 *
 * This function should only be used by omcid.  This function will
 * allow all read-only parameters in this object to be written to.
 * This function does not trigger the RCL handler function.  The new
 * object will have its parameters checked against the restrictions in
 * the data model, and then be placed directly into the MDM.
 *
 * @param mdmObj   (IN) Pointer to the modified GPON object.
 * @param iidStack (IN) The instance information of the GPON object
 *                      to be written to.
 *
 * @return CmsRet enum.
 */
CmsRet cmsObj_setGponEntityId(const void *mdmObj, const InstanceIdStack *iidStack);
