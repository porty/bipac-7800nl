...
do
    ...
    do
        if [ "${NAME[$i]:0:5}" != ".init" ]; then
    	    masksMatch "$oneMask" "${FLAGS[$i]}" "$i" $4	 
            if [ "$?" -eq "1" ]; then
...
`sed s/'#define FAP_CORE_SIZE .*'/"#define FAP_CORE_SIZE $coreSize"/g $2/opensource/include/bcm963xx/fap_mod_size.h > $2/opensource/include/bcm963xx/fap_mod_size1.h`
`mv -f $2/opensource/include/bcm963xx/fap_mod_size1.h $2/opensource/include/bcm963xx/fap_mod_size.h`
`sed s/'#define FAP_INIT_SIZE .*'/"#define FAP_INIT_SIZE $initSize"/g $2/opensource/include/bcm963xx/fap_mod_size.h > $2/opensource/include/bcm963xx/fap_mod_size1.h`
`mv -f $2/opensource/include/bcm963xx/fap_mod_size1.h $2/opensource/include/bcm963xx/fap_mod_size.h`
