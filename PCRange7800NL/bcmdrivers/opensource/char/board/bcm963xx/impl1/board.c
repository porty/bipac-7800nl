...
/* Automated Voltage Scaling is only characterized for 6362 at this time */
#if defined(CONFIG_BCM96362)
...
/* 1V2 is set by default to 1.225. On a very large sample, the lowest
   voltage measured on 1V2 was not 1.225, but 1.2 (excluding a very
   low percentage of parts). The 1.2V is allowed to go down to 1.0875,
   which consists of 9 steps of 0.0125 below 1.2V. */
#define VREG_VSEL1P2_LOWER_BOUND ((VREG_VSEL1P2_MIDDLE+1) - 9)
#endif
#if defined(CONFIG_BCM96816)
...
static void brcm_automatic_voltage_scaling(void)
{
    ...
    if (RingOscCtrl1 & RING_OSC_IRQ)
    {
        ...
        if (ring_osc_select == RING_OSC_MAX)
        {
            ...
            if (max_count < RING_OSC_LOWER_BOUND)
            {
            ...
            else if (max_count > RING_OSC_UPPER_BOUND)
            {
...
static int brcm_board_timer( void *data )
{
    ...
    {
        brcm_automatic_voltage_scaling();
...
int kerSysGetWlanSromParams( unsigned char *wlanParams, unsigned short len)
{
    ...
    readNvramData(pNvramData);
    memcpy( wlanParams, (char *)pNvramData + ((size_t) &((NVRAM_DATA *)0)->wlanParams),
        len );
    kfree(pNvramData);
    return 0;
}
                       ...
                       unsigned int command, unsigned long arg )
{
    ...
    case BOARD_IOCTL_FLASH_WRITE:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            ...
            case BCM_IMAGE_CFE:
                ...
                if ((readNvramData(pTmpNvramData) != 0) || (BpSetBoardId(pTmpNvramData->szBoardId) != BP_SUCCESS) || (BpSetVoiceBoardId(pTmpNvramData->szVoiceBoardId) != BP_SUCCESS))
#else
                if ((readNvramData(pTmpNvramData) != 0) || (BpSetBoardId(pTmpNvramData->szBoardId) != BP_SUCCESS))
#endif
                    writeNvramData(pNvramData);
            ...
            case BCM_IMAGE_FS:
                if( (ret = flashFsKernelImage(ctrlParms.string, ctrlParms.strLen)) == 0 )
                    //kerSysMipsSoftReset();   //simon 2010.02 (modify for upgrade firmware reboot step)
                break;
    ...
    case BOARD_IOCTL_SET_PROID:                                                      //simon 2008.06(Set & Get productId)
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            NVRAM_DATA nvramData;
            readNvramData(&nvramData);
            strcpy(nvramData.szProductId,ctrlParms.string);
    ...
    case BOARD_IOCTL_GET_PROID:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            NVRAM_DATA nvramData;
            readNvramData(&nvramData);
            strcpy(ctrlParms.string,nvramData.szProductId);
            ret = 0;
        }
        else {
            ret = -EFAULT;
    ...
    case BOARD_IOCTL_SET_VENDORID:                                                      //simon 2009.05(Set & Get vendorId)
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            NVRAM_DATA nvramData;
            readNvramData(&nvramData);
            strcpy(nvramData.szVendorId,ctrlParms.string);
            writeNvramData(&nvramData);
            ret = 0;
        }
        else {
    ...
    case BOARD_IOCTL_GET_VENDORID:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            NVRAM_DATA nvramData;
            readNvramData(&nvramData);
            strcpy(ctrlParms.string,nvramData.szVendorId);
            ret = 0;
        }
        else {
            ret = -EFAULT;
...
{
    printk("\n*** ");
    kernel_thread(restore_to_default_thread, NULL, 0);
    return IRQ_HANDLED;
}
