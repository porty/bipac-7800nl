...
typedef struct BcmPktDma_XtmRxDma
{
...
} BcmPktDma_XtmRxDma, RXBDINFO, *PRXBDINFO;
...
typedef struct BcmXtm_RxDma {
    ...
    unsigned char   *buf_pool[NR_RX_BDS_MAX];
    /* FIXME: SKB Pool must be dynamically allocated */
    unsigned char    skbs[(400 * SKB_ALIGNED_SIZE) + 0x10];
    int              rxIrq; 
    int              channel; 
} BcmXtm_RxDma;
              
typedef struct BcmPktDma_XtmTxDma
{
    UINT32 ulPort;   /* the channel within the device context */
...
} BcmPktDma_XtmTxDma, TXQINFO, *PTXQINFO;
...
typedef struct bcmxtmrt_dev_context
{
    ...
    uint32   txDmaKeys[MAX_TRANSMIT_QUEUES*NR_TX_BDS];
    int      txDmaSources[MAX_TRANSMIT_QUEUES*NR_TX_BDS];
    uint32   txDmaAddresses[MAX_TRANSMIT_QUEUES*NR_TX_BDS];
#endif
