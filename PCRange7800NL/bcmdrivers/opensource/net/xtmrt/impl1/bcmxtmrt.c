...
/*howard add on 20100120*/
#include "billion_autoscan_api.h"
UINT32 Glob_PotocolType[PACKET_TYPE_MAX] = {0,0,0,0,0,0};
/*howard end*/
...
int bcmxtmrt_xmit(pNBuff_t pNBuff, struct net_device *dev);
...
int bcmxtmrt_xmit( pNBuff_t pNBuff, struct net_device *dev )
{
   ...
   PBCMXTMRT_DEV_CONTEXT pDevCtx = dev->priv;
	...
	/*howard add on 20100119*/
	UINT32 mode = DEFAULTMODE;
    	GetAndSetMode(GETMODE,&mode);
	//printk("mode = %d   pNBuffSkb->mark = %d\n",mode,pNBuffSkb->mark);
    	if(mode == AUTOSCANMODE &&  pNBuffSkb != NULL && !(pNBuffSkb->mark & AUTOSCANMARK))
    	{
		//printk("bcmxtmrt_xmit: driver is doing autoscan, so can't send packet\n");
        ...
        if( pDevCtx->ulTxQInfosSize )
        {
            ...
            if( pTqi->ulFreeBds && QueuePacket(pGi, pTqi, isAtmCell) )
            {
                ...
                if ( ulRfc2684_type && pNBuffSkb != NULL && !(pNBuffSkb->mark & AUTOSCANMARK)) // howard add !(pNBuffSkb->mark & AUTOSCANMARK)
                {
											...
											int trailerDelLen)
{
...
#else
   {
	...
	UINT32 mode = DEFAULTMODE;	
	GetAndSetMode(GETMODE,&mode);
   	if(mode == AUTOSCANMODE)
   	{
          UINT32 bxStatus = XTMSTS_ERROR;
          scanpvc_parse_rxpacket(pBuf,&bxStatus,pDevCtx,Glob_PotocolType);
#if 0
          FlushAssignRxBuffer((PRXBDINFO) pFkb->recycle_context,pucData,pucData);
          return PACKET_NORMAL;
#endif
   	}
   	/*howard end*/
...
/*howard add on 20100115*/
    case XTMRT_CMD_DO_AUTOSCAN_PVC:
	if(((PXTMCFGDRV_AUTOSCAN_PVC)pParm)->start){
		memset(Glob_PotocolType,0,PACKET_TYPE_MAX*4);
		nRet = DoSendAutoScanPkt( pDevCtx , (PXTMCFGDRV_AUTOSCAN_PVC) pParm);
	}
	else{
		memset(((PXTMCFGDRV_AUTOSCAN_PVC)pParm)->PotocolType,0,PACKET_TYPE_MAX*4);
		nRet = DoGetAutoScanReport(((PXTMCFGDRV_AUTOSCAN_PVC)pParm)->PotocolType,Glob_PotocolType);
	}
	break;

    case XTMRT_CMD_SET_AUTOSCAN_FLAG:
	pDevCtx->uAutoScanFlags = *(UINT32*) pParm;
	GetAndSetMode(SETMODE,(UINT32*) pParm);
	if(pDevCtx->uAutoScanFlags){
                pDevCtx->uBackUp_ulTrafficType = pDevCtx->Addr.ulTrafficType;
                pDevCtx->Addr.ulTrafficType = TRAFFIC_TYPE_ATM;
                if((pDevCtx->ulFlags & CNI_HW_ADD_HEADER)){
                        pDevCtx->ulFlags &= ~CNI_HW_ADD_HEADER;
                        pDevCtx->uBackUp_HW_ADD_HEADER = 1;
                }
		if((pDevCtx->ulFlags & CNI_HW_REMOVE_HEADER)){
                	pDevCtx->ulFlags &= ~CNI_HW_REMOVE_HEADER;
			pDevCtx->uBackUp_HW_REMOVE_HEADER = 1;
		}
	}else
	{
		pDevCtx->Addr.ulTrafficType = pDevCtx->uBackUp_ulTrafficType;
                if(pDevCtx->uBackUp_HW_REMOVE_HEADER)
                        pDevCtx->ulFlags |= CNI_HW_ADD_HEADER;
		if(pDevCtx->uBackUp_HW_REMOVE_HEADER)
			pDevCtx->ulFlags |= CNI_HW_REMOVE_HEADER;
	}
...
static int DoCreateDeviceReq( PXTMRT_CREATE_NETWORK_DEVICE pCnd )
{
         ...
         pCnd->szNetworkDeviceName, ether_setup )) != NULL )
    {
        ...
        printk("[%s.%d]: register_netdev: %s\n", __func__, __LINE__,dev->name);
        nRet = register_netdev(dev);
        printk("[%s.%d]: register_netdev %s done\n", __func__, __LINE__,dev->name);
        if (nRet == 0) 
        {
            UINT32 i;
            for( i = 0; i < MAX_DEV_CTXS; i++ )
...
static int DoDeleteDeviceReq( PBCMXTMRT_DEV_CONTEXT pDevCtx )
{
    ...
    /*howard add on 20100128*/
    for ( i= 0; i < MAX_MATCH_IDS; i++ )
        if( pGi->pDevCtxsByMatchId[i] != NULL)
	    break;
    if (i >= MAX_MATCH_IDS) // no device
    {
	volatile DmaChannelCfg *pRxDma;
