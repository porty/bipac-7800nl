...
static struct option longopts[] = 
{
    ...
    { "productid",  required_argument, 0, 'd'},  // sarah, 20091127
    { "vendorid",   required_argument, 0, 'r'},
 ...
 ***************************************************************************/
int fillInOutputBuffer(char *szBoardId, char *szVoiceBoardId, int numOfMac, unsigned char *ucaBaseMac,
                       int tpNum, int psiSize, unsigned char *gponSerialNbr,
                       unsigned char *gponPasswd, unsigned long ulOutputSize,
                       char *pszIn, char *pszOut, char *szProductId , char *szVendorId) // sarah modify, 20091127
{
    int nLen;
    ...
    strncpy(nvramData.szVoiceBoardId, szVoiceBoardId, NVRAM_BOARD_ID_STRING_LEN);
    strncpy(nvramData.szProductId, szProductId, NVRAM_PRODUCT_ID_STRING_LEN); //sarah, 20091127
    if(strstr(szVendorId,"NULL")==0)
       strncpy(nvramData.szVendorId, szVendorId, NVRAM_VENDOR_ID_STRING_LEN);
...
void createImage(char *szBoardId,
    ...
    char *inputFile,
    char *outputFile,
    char *szProductId,                                  //simon 2008.06 (add productid) 
    char *szVendorId
) // sarah modify, 20091127
{
    FILE *hInput = NULL;
    struct stat StatBuf;
    ...
    if (fread( pszIn, sizeof(char), StatBuf.st_size, hInput ) == StatBuf.st_size)
    {
        ...
        if( verifyTag((PFILE_TAG)pszIn) == 0)
        {
            ...
            if ((nLen = fillInOutputBuffer(szBoardId, szVoiceBoardId, numOfMac, ucaBaseMac,
                        tpNum, psiSize, gponSerialNbr, gponPasswd, ulOutputSize, pszIn, pszOut, szProductId, szVendorId)) > 0) // sarah modify, 20091127
            {
                /* Open output file. */
...
int main (int argc, char **argv)
{
    ...
    char szProductId[BP_BOARD_ID_LEN] = ""; // sarah, 20091127                      //simon 2008.06 (add productid)
    char szVendorId[BP_VENDOR_ID_LEN] = ""; // sarah, 20091127
    ...
    printf("\tProduct id              : %s\n", szProductId);   // sarah add, 20091127                 //simon 2008.06 (add productid)
    printf("\tVendor id               : %s\n", szVendorId);
    ...
    createImage(szBoardId, szVoiceBoardId, numOfMac, ucaBaseMacAddr, tpNum, psiSize,
                gponSerialNbr, gponPasswd, inputFile, outputFile, szProductId, szVendorId); // sarah modify, 20091127
