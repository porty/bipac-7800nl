...
static struct option longopts[] = {
    ...
    { "chip", 1, 0, 0},
    { "board", 1, 0, 0},
    { "product", 1, 0, 0},   //simon 2008.06(add product), sarah copy, 20091127
    { "vendor", 1, 0, 0},
    { "blocksize", 1, 0, 0},
    { "output", 1, 0, 0},
    { "cfefile", 1, 0, 0},
    { "rootfsfile", 1, 0, 0},
    { "kernelfile", 1, 0, 0},
    { "imgtype", 1, 0, 0},   //simon 2008.05(add imgtype), sarah copy, 20091127
    { "include-cfe", 0, 0, 'i'},
    { 0, 0, 0, 0 }
};
...
static char usage[] = "Usage:\t%s {-h|--help|-v|--version}\n"
                    ...
                    "\t--board <boardid> --product <productid> --vendor <vendorid>\n" //simon 2008.06(add productid), sarah copy, 20091127
                    "\t--blocksize {64|128}\n"
                    "\t--output <filename> --cfefile <filename>\n"
                    "\t[--rootfsfile <filename> --kernelfile <filename> [-i|--include-cfe]]\n"
                    "\t--imgtype <imagetype> -- image type{R|S}\n";  //simon 2008.05(add imgtype), sarah copy, 20091127
...
static void build_image(int littleendian, char *chipid, char *board, unsigned int blksize, 
    char *outputfile, char *cfefile, char *rootfsfile, char *kernelfile, int includecfe, char *product, char *vendor, char *imgtype)  //simon 2008.05, sarah copy, 20091127
{
    ...
    if (strcmp(imgtype,BCM_TAG_VER_2) == 0)                                  //simon 2008.05 start(add strncpy imgtype)
        strcpy(tag.tagVersion, BCM_TAG_VER_2);
    else if (strcmp(imgtype,BCM_TAG_VER) == 0)
    strcpy(tag.tagVersion, BCM_TAG_VER);                                 //simon END
    ...
    strncpy(tag.productId, product, PRODUCT_ID_LEN -1);                  //simon 2008.06(add strncpy productId)
    if(strstr(vendor,"NULL")==0)
       strncpy(tag.vendorId, vendor, VENDOR_ID_LEN -1);
    if (littleendian)
...
#ifdef DEBUG
    printf ("imgtype: %s\n", imgtype);                                    //simon 2008.05(add imgtype)
    printf ("tagVersion %s\n", tag.tagVersion);
    printf ("signiture_1 %s\n", tag.signiture_1);
    printf ("signiture_2 %s\n", tag.signiture_2);
    printf ("chipId %s\n", tag.chipId);
    printf ("boardId %s\n", tag.boardId);
    printf ("productId %s\n", tag.productId);                            //simon 2008.06(add productId)
    printf ("vendorId %s\n", tag.vendorId);				 //simon 2009.05(add vendorId)
    printf ("bigEndian %s\n", tag.bigEndian);
...
int main (int argc, char **argv)
{
  ...
  int option_index = 0;
  char cfefile[256]="", rootfsfile[256]="", kernelfile[256]="", outputfile[256]="", imgtype[4]="";            //simon 2008.05(add imgtype)
  char boardid[BOARD_ID_LEN]="";
  char productid[PRODUCT_ID_LEN]=""; //simon 2008.06 (add productid)
  char vendorid[VENDOR_ID_LEN]=""; //simon 2009.05 (add vendorid)
  char chipid[CHIP_ID_LEN]="";
  unsigned int blksize = 0;

  progname = argv[0];
  chipid[0] = boardid[0] = cfefile[0] = kernelfile[0] = rootfsfile[0] = outputfile[0] 
  = imgtype[0] = productid[0] = vendorid[0] = '\0' ;
                                                                                   //simon 2008.05(add imgtype & productid)
  while ((optc = getopt_long (argc, argv, "hvli", longopts, &option_index)) != EOF) {
      switch (optc){
        case 0:
          ...
          // sarah, 20091127
          else if (strcmp(longopts[option_index].name, "product") == 0)
             strcpy(productid, optarg);
          else if (strcmp(longopts[option_index].name, "vendor") == 0)
             strcpy(vendorid, optarg);
          else if (strcmp(longopts[option_index].name, "imgtype") == 0)          
             strcpy(imgtype, optarg);
          // sarah end, 20091127
  ...
  if (h) {
      ...
      fputs ("  --product  <product name>       Product name\n", stderr);//simon 2008.06 (add product name)
      ...
      fputs ("  --imgtype <filename>        firmware image type\n", stderr); //simon 2008.05 (add imgtype)
...
#ifdef BILLION_PROD
  if (strlen(chipid) == 0 || strlen(boardid) == 0 || strlen(outputfile) == 0 || strlen(productid) == 0 || strlen(vendorid) == 0 
      || strlen(cfefile) == 0 || (blksize != 64*1024 && blksize !=128*1024) ||strlen(imgtype) == 0)       //simon 2008.05(add imgtype & productid)
#else
  ...
  if (strlen(productid) > 15)   //simon 2008.06 (add productid)
  {
      fprintf (stderr, "Maximum product id length is 15 characters\n\n");
      fprintf (stderr, usage, progname);
      exit (1);
  }                             //simon END
  if (strlen(vendorid) > 11)   //simon 2008.06 (add vendorid)
  {
      fprintf (stderr, "Maximum vendor id length is 11 characters\n\n");
      fprintf (stderr, usage, progname);
      exit (1);
  }

  build_image(littleendian, chipid, boardid, blksize, outputfile, 
              cfefile, rootfsfile, kernelfile, includecfe, productid, vendorid, imgtype);
					                                             //simon 2008.06(add imgtype & productid)
  exit(0);
}
